"""
Pydantic v2 schemas and data definitions for Hplc Chromatography Peak Resolver.
...
class SystemTaskPayload(BaseModel):
    ...
    def reject_nan_inf(cls, v: float) -> float:
        if math.isnan(v) or math.isinf(v):
            raise ValueError("Metric values must be finite numbers (NaN/Inf not allowed)")
        return v
...
class ConsensusDossier(BaseModel):
    dossier_id: str
    system_slug: str = "hplc-chromatography-peak-resolver"
